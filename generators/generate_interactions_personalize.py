...
def generate_data():
    """Generate items.csv, users.csv straight from input yaml files - and interactions.csv by simulating some
    shopping behaviour."""
    ...
    products_dataset_df = products_dataset_df.rename(columns={'id': 'ITEM_ID',
                                                              'category': 'CATEGORY',
                                                              'style': 'STYLE'})
    if SKIP_EXISTING and Path(out_items_filename).exists():
        print(f'The target items file ({out_items_filename}) already exists. '
              f'Without inspecting the contents, we are skipping it.')
    else:
    ...
    if SKIP_EXISTING and Path(out_users_filename).exists():
        print(f'The target users file ({out_users_filename}) already exists. '
              f'Without inspecting the contents, we are skipping it.')
    else:
    ...
    if SKIP_EXISTING and Path(out_interactions_filename).exists():
        print(f'The target interactions file ({out_interactions_filename}) already exists. '
              f'Without inspecting the contents, we are skipping it.')
    else:
                ...
                for _ in range(num_interaction_sets_to_insert):
                    ...
                    if product_added_count < int(product_viewed_count * product_added_percent):
                        ...
                        if discounted:
                            f.writerow([product['id'],
                                        user['id'],
                                        'DiscountedProductAdded',
                                        this_timestamp])
                            interactions += 1
                            discounted_product_added_count += 1
                            # we do not count this as an interaction for the purpose of building our dataset
