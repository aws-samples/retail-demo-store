...
export default {
  ...
  methods: {
    async fetchCustomerRoute() {
      const customerRouteGeojson = (await LocationRepository.get_cstore_route()).data
    ...
    async fetchStoreLocation() {
      const storeLocationGeojson = (await LocationRepository.get_cstore_location()).data
